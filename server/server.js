...
app.post('/api/projects', authenticateToken, (req, res) => {
  const { id, name, thumbnail, nodes, edges, viewport, updatedAt, tags } = req.body;
  const projectId = id || uuidv4();
  const ts = updatedAt || Date.now();

  const sql = `INSERT OR REPLACE INTO projects (id, userId, name, thumbnail, nodes, edges, viewport, updatedAt, tags) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  const params = [
      projectId, 
      req.user.id,
      name, 
